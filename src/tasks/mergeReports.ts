...
task(TASK_GAS_REPORTER_MERGE)
  ...
  .addVariadicPositionalParam(
    "input",
    "A list of gasReporterOutput.json files generated by eth-gas-reporter. " +
  ...
  .setAction(async (taskArguments, hre) => {
    ...
    const taskArgs = uniq(taskArguments.input.map(globSync).flat());
    ...
    const result = await hre.run(TASK_GAS_REPORTER_MERGE_REPORTS, { files });
    const warnings = await setGasAndPriceRates(result.options);
    const data = new GasData(result.methods, result.deployments);
