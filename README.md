# :fuelpump: buidler-gas-reporter :fuelpump:

An [eth-gas-reporter](http://getbuidler.com) plugin for [buidler](http://getbuidler.com).
...
The options include an `enabled` key so you can toggle gas reporting on and off using shell
...
This plugin extends the built-in `test` task. Gas reports are generated by default with:
```
npx buidler test
```
