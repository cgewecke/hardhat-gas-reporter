...
| etherscan                                  |  _string_  |      -     | [API key][4] to use when fetching live gasPrice and fee data from both L1 & L2 networks(Optional, see [Supported Networks][6])                                                                                                                                      |
...
An API key for the networks this plugin auto-configures via the `L1` and `L2` options are available at the links below. In many cases this isn't required - you'll only need to set one if you start seeing rate-limit warnings.
